%-------------------------------------------------------------------------%
%    !!! Only order input side in this subversion !!!
% File: lineOrder
% Version: 1.0_outputOnly
...
%     set_param(xListExs{ii},'Position',[i_inx+boundInLeft-val_len*5 i_iny-10 i_inx+boundInRight i_iny+10]);
...
for ii=1:length(xListExs)
    ...
    i_outy=i_out(2);
    i_outx=i_out(1);
    if strcmp(get_param(xListExs{ii},'BlockType'),'Goto')
        tg=get_param(xListExs{ii},'GotoTag');
